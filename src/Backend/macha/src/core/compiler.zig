...
fn bucketLamps(t: std.mem.Allocator, lamp_gate: []const u32, gate_count: usize) std.mem.Allocator.Error!LampColumns {
    ...
    std.debug.assert(total == lamp_gate.len);
...
fn buildGatesAndLamps(
    ...
    for (gates, 0..) |*g, gi| {
        ...
        var normal_lamp_count: usize = 0;
        var has_fault_lamp = false;
        for (column) |li| {
            if (metadata.lamp_kind[li] == .fault) has_fault_lamp = true else normal_lamp_count += 1;
        }
        const normal_lamps = try t.alloc(usize, normal_lamp_count);
        var normal_i: usize = 0;
        for (column) |li| {
            if (metadata.lamp_kind[li] != .fault) {
                normal_lamps[normal_i] = li;
                normal_i += 1;
            }
        }

        g.lamps = try lampPtrs(a, lamps, normal_lamps);

        if (has_fault_lamp) {
            // A fault gate with no normal lamp would divide by zero in the
            // sim's compound roll; such a gate can never fire, so reject it.
            if (normal_lamp_count == 0) return error.FaultGateWithoutLamp;
            g.class = if (normal_lamp_count == 1) .fault_1 else .fault_n;
            g.state = undefined;
            ...
            g.class = switch (kind) {
                .and_, .nand, .or_, .nor => switch (normal_lamp_count) {
                    1 => .gate_1,
                    2 => .gate_2_and,
                    else => .gate_n_and,
                },
                .xor, .xnor => switch (normal_lamp_count) {
                    1 => .gate_1,
                    2 => .gate_2_xor,
                    else => .gate_n_xor,
                },
            ...
            if (kind == .or_ or kind == .nor) {
                ...
                for (normal_lamps) |li| metadata.lamp_initial[li] = !metadata.lamp_initial[li];
            }
            g.state = initialState(g.class, normal_lamps, metadata.lamp_initial);
        }
    }
...
fn buildWireTargets(
    ...
    for (wiring.wires) |rec| {
        ...
        var count: usize = 0;
        const output_port_base: i32 = @intCast(wiring.input_ports.len);
        for (rec.fanout) |id| {
            if (id >= lamp_base and id < gate_base) {
                // target is lamp
                count += 1;
            } else if (id >= output_port_base and id < lamp_base) {
        ...
        std.debug.assert(count == rec.fanout.len);
        ...
        for (rec.fanout, 0..) |id, i| {
            if (id >= lamp_base and id < gate_base) {
                ...
                const gate_class = gates[metadata.lamp_gate[li]].class;

                if (metadata.lamp_kind[li] == .fault) {
                    targets[i] = .{ .fault_gate = metadata.lamp_gate[li] };
                }
                else if (gate_class == .fault_1 or gate_class == .fault_n) {
                    targets[i] = .{ .lamp_on_fault = li };
                }
                else {
                    targets[i] = .{ .lamp = li };
                }
            } else {
                targets[i] = .{ .output_port = id - output_port_base };
            }
        }
...
fn initialState(class: Graph.Gate.Class, normal_lamps: []const usize, lamp_initial: []const bool) bool {
    return switch (class) {
        .gate_1 => lamp_initial[normal_lamps[0]],
        .gate_2_and => lamp_initial[normal_lamps[0]] and lamp_initial[normal_lamps[1]],
        .gate_2_xor => lamp_initial[normal_lamps[0]] != lamp_initial[normal_lamps[1]],
        .gate_n_and, .gate_n_xor => {
            var on: u32 = 0;
            for (normal_lamps) |li| {
                if (lamp_initial[li]) on += 1;
            }
            return if (class == .gate_n_and) on == @as(u32, @intCast(normal_lamps.len)) else on == 1;
        },
